> Other languages: [Russian](https://github.com/bespoyasov/ranking-project/blob/master/docs/ru.md).

# Frontend Clean Architecture

A React + TypeScript example app built using the clean architecture in a functional(-ish) way.

- [Working app](https://bespoyasov.ru/showcase/ranking-project/en/)
...
In this application, the shared kernel includes global type annotations that can be accessed anywhere in the app and by any module. Such types are collected in [`shared-kernel.d.ts`](https://github.com/bespoyasov/ranking-project/blob/master/src/shared-kernel.d.ts).

### Dependency in the Domain

The [`createOrder`](https://github.com/bespoyasov/ranking-project/blob/master/src/domain/order.ts#L15) function uses the library-like function `currentDatetime` to specify the order creation date. This is not quite correct, because the domain should not depend on anything.
...
The order creation function [`orderProduct`](https://github.com/bespoyasov/ranking-project/blob/master/src/application/orderProducts.ts#L24) itself is framework-independent right now and can't be used and tested in isolation from React. The hook wrapper though is only used to provide the use case to components and to inject services into the use case itself.
...
In the [application layer](https://github.com/bespoyasov/ranking-project/blob/master/src/application/orderProducts.ts) we inject services by hand:
